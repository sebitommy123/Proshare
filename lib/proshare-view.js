...
export default class ProshareView {
    ...
    login.onclick = evt => {
      ...
      instance.parent.sendRequest("noTrustLogin", {
        ...
        if(res == "no"){
          ...
          instance.element.innerHTML += `<div> Hello <a href='#' style='display: inline;'>${username.value}</a></div>`;
          ...
          rlButton.onclick = ()  => {
            ...
            rlButton.onclick = ()  => {
              instance.fetchProjects(instance.showTreeView);
            };

            instance.element.appendChild(rlButton);
  ...
  showTreeView(projects, instance){
    ...
    // let topLevelWrapper = document.createElement("DIV");
          ...
          atom.workspace.open(`Proshare/${project.name}/${file.name+"."+file.fileType}`).then(textEditor => {
            ...
            .then(() => {
              ...
              textEditor.onDidDestroy(e => {
                ...
                // atom.notifications.addError("Disregarding changes ):", {icon: "gift"});
...
<span class="name icon icon-file-directory" title="Proshare">
  Proshare
    ...
    // instance.element.appendChild(topLevelWrapper);
